...
/**
 * Owns all active-company state + lifecycle: the eager token-rescoping
 * `switchTo` (serialized via `switchChain`), the `listMine` bootstrap (`load`),
 * the mount + token-transition effects, and `setActiveCompany`. Returns the
 * assembled {@link CompanyContextValue} for the provider to publish.
 */
